/*
 *
 * ModifiedPerceptron.java utumno.mope.scripts.MoPeTrainScripts
 *
...
public class MoPeTrainScripts {
	...
	public static void main(String[] args) {
		...
		String exe;
		if(linuxMode){
			exe = " java -Xmx3072m -server -Xoptimize -classpath \""+prefix+"mope.jar\" utumno.mope.cmd.ClassVectorBuilderAdjusterCmd -iter 100 -inData ";
		}
		else{
			exe = " java -Xmx3072m -server -Xoptimize -classpath \""+prefix+"mope.jar\" utumno.mope.cmd.ClassVectorBuilderAdjusterCmd -iter 100 -inData ";
		}
		...
		try {
			for(String dataset : datasets){
				...
				if(s.length == 2){
					bw = new BufferedWriter(new FileWriter(prefix0+dataset+".train.mope."+ (linuxMode?"sh":"bat")));
				...
				for(String className : classesNames){
					if(linuxMode){
						...
						String exe2 = time+prefix+prefix2+"train"+fileSeparator+className+".train.mope.txt2\" " + exe;
