/*
 *
 * ModifiedPerceptron.java utumno.mope.scripts.BatchPerceptronTestScripts
 *
...
public class BatchPerceptronTestScripts {
	...
	public static void main(String[] args) {
		...
		String exe;
		if(linuxMode){
			exe = " java -Xmx3584m -server -Xoptimize -classpath \""+prefix+"rh.jar\" utumno.perceptron.BatchPerceptronEvaluatorCmd -inData ";
		}
		else{
			exe = " java -Xmx1600m -server -Xoptimize -classpath \""+prefix+"rh.jar\" utumno.perceptron.BatchPerceptronEvaluatorCmd -inData ";
		}
		...
		try {
			for(String dataset : datasets){
				...
				if(s.length == 2){
					bw = new BufferedWriter(new FileWriter(prefix0+dataset+".test.bape." + (linuxMode?"sh":"bat")));
				...
				for(String className : classesNames){
					if(linuxMode){
						...
						String exe2 = time+prefix+prefix2+"test"+fileSeparator+className+".test.bape.txt2\" " + exe;
						String command = exe2 + "\"" + prefix + prefix2 + "test"+fileSeparator + className + ".test.svmf\" -inModel \""+prefix+prefix2+"train"+fileSeparator+className+".bape.model\"" + (linuxMode?";":"");
						bw.write(command + (linuxMode?"":"\r") +"\n");
					}
					else{
						String command = exe + "\"" + prefix + prefix2 + "test"+fileSeparator + className + ".test.svmf\" -inModel \""+prefix+prefix2+"train"+fileSeparator+className+".bape.model\"" + (linuxMode?";":"");
						bw.write(command + (linuxMode?"":"\r") +"\n");
					}
				}
