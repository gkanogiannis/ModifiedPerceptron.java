/*
 *
 * ModifiedPerceptron.java utumno.mope.Adjusting_new
 *
...
public class Adjusting_new {
	...
	public Adjusting_new(ClassVector classVector, ArrayList<DocumentVector> documentVectors, int iterations, PrintStream printstream, BufferedWriter outTxt, boolean verbose){
		...
		try{
			...
			do{
				...
				if(!NAHeap.isEmpty() && !PRHeap.isEmpty()){
		            ...
		            //beta = (b2 - b1) / (b3 + b4 - b5);
					beta = - 2.0f * (classVector.similarityWithDocumentVector(e))/(e.similarityWithDocumentVector(e));
		            ...
		            float bPos = classVector.similarityWithDocumentVector(PR);
		            float bNeg = classVector.similarityWithDocumentVector(NA);
		            
		            ClassVector bC = new ClassVector();
					bC.setNodes(new VectorNode[classVector.getNodes().length]);
					bC.addWithDocumentVector(PR, 0.5f);
					bC.addWithDocumentVector(NA, 0.5f);
					DocumentVector b = new DocumentVector(bC.getNodes());
					classVector.setPosThresholdVector(b.clone());
					classVector.setNegThresholdVector(b.clone());
		            
					classVector.setThreshold((bPos+bNeg)/2.0f);
				}
					...
					float bPos = classVector.similarityWithDocumentVector(classVector.getPosThresholdVector());
		            float bNeg = classVector.similarityWithDocumentVector(NA);
		            
		            ClassVector bC = new ClassVector();
					bC.setNodes(new VectorNode[classVector.getNodes().length]);
					bC.addWithDocumentVector(classVector.getPosThresholdVector(), 0.5f);
					bC.addWithDocumentVector(NA, 0.5f);
					DocumentVector b = new DocumentVector(bC.getNodes());
					classVector.setPosThresholdVector(b.clone());
					classVector.setNegThresholdVector(b.clone());
					...
					float bPos = classVector.similarityWithDocumentVector(PR);
		            float bNeg = classVector.similarityWithDocumentVector(classVector.getNegThresholdVector());
		            
		            ClassVector bC = new ClassVector();
					bC.setNodes(new VectorNode[classVector.getNodes().length]);
					bC.addWithDocumentVector(PR, 0.5f);
					bC.addWithDocumentVector(classVector.getNegThresholdVector(), 0.5f);
					DocumentVector b = new DocumentVector(bC.getNodes());
					classVector.setPosThresholdVector(b.clone());
					classVector.setNegThresholdVector(b.clone());
