/*
 *
 * ModifiedPerceptron.java utumno.mope.utils.BatchPerceptronMicroMacroResults
 *
...
public class BatchPerceptronMicroMacroResults {
	...
	public static void main(String[] args) {
		...
		try{
			...
			for(File f : files){
				try{
				if(f.getName().endsWith(".test.bape.txt")){
					i++;
			...
			double macroBEP = 100.0 * BEP / (double) i;
			if(Double.isNaN(macroBEP)) macroBEP = 0.0;
			double macroROCArea = 100.0 * ROCArea / (double) i;
			if(Double.isNaN(macroROCArea)) macroROCArea = 0.0;
			double macroAvPr = 100.0 * AvPr / (double) i;
		...
		try{
			...
			for(File f : files){
				if(f.getName().endsWith(".train.bape.txt")){
					i++;
					BufferedReader br = new BufferedReader(new FileReader(f));
					br.readLine();br.readLine();br.readLine();br.readLine();br.readLine();
